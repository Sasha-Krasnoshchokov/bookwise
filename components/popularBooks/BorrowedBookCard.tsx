...
const BorrowedBookCard = ({
	...
	const datesToDue = Math.ceil(
		(new Date(dueDate).getTime() - new Date().getTime()) / ONE_DAY_IN_MS
	);
	const daysText = datesToDue > 1 ? 'days' : 'day';
	const coverColorRGB = hexToRgba(coverColor || '')
		.split('(')[1]
		.split(')')[0]
		.split(',')
		.slice(0, 3);
	...
	const isOverdue = !isReturned && datesToDue <= 0;

	const bookStatus = isOverdue ? 'overdueReturn' : isReturned ? 'returned' : 'default';

	const statusConfig = {
		default: {
			icon: '/icons/calendar.svg',
			alt: 'calendar',
			text: `${datesToDue} ${daysText} left to due`,
			classNames: 'text-light-100 text-sm',
		},
		overdueReturn: {
			icon: '/icons/warning.svg',
			alt: 'warning',
			text: 'Overdue Return',
			classNames: 'text-red-warning text-sm',
		},
		returned: {
			icon: '/icons/tick.svg',
			alt: 'checked',
			text: `Returned on ${new Date(returnDate).toDateString().split(' ').slice(1).join(' ')}`,
			classNames: 'text-light-100 text-sm',
		},
	};
		...
		<li className='relative flex flex-col gap-5 p-5 xl:w-[48%] w-full min-h-[200px] rounded-md gradient-vertical'>
			{(isReturned || isOverdue) && (
				<div className='absolute top-[-6px] left-[-6px]'>
					<Image
						src={statusConfig[bookStatus].icon}
						alt={statusConfig[bookStatus].alt}
						width={26}
						height={26}
					...
					<div className='flex flex-row justify-between pt-2'>
						<div className='flex flex-row items-center gap-2'>
							<Image
								...
								className='object-contain'
							/>
							<p className={statusConfig[bookStatus].classNames}>{statusConfig[bookStatus].text}</p>
						</div>
						<div
							...
							<Receipt
								fill={coverColorRGB.every((color) => parseInt(color) > 180) ? '#000' : '#fff'}
							/>
						</div>
					</div>
				</div>
...
const Receipt = ({ fill }: { fill?: string }) => {
	return (
		<svg
			width='18'
			height='18'
